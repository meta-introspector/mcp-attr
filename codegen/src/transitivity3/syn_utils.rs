...
use quote::quote;
use syn::{
    punctuated::Punctuated, DeriveInput, GenericArgument, Path, PathArguments, PathSegment, Result,
...
pub fn get_element_of_path<'a>(path: &'a Path, ns: &[&[&str]], name: &str) -> Option<&'a Type> {
    if let PathArguments::AngleBracketed(args) = get_arguments_of_path(path, ns, name)? {
        if args.args.len() == 1 {
...
pub fn get_element_of_ty<'a>(ty: &'a Type, ns: &[&[&str]], name: &str) -> Option<&'a Type> {
